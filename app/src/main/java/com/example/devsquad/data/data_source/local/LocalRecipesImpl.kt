...
class LocalRecipesImpl(private val recipeDao: RecipeDao = RecipeDatabase.getInstance(MyApp.applicationContext).recipeDao(), private val categoryDao: CategoryDao = RecipeDatabase.getInstance(MyApp.applicationContext).categoryDao()) {
    suspend fun getRecipeById(id: String) = recipeDao.getRecipeById(id)
    suspend fun getAllRecipes() = recipeDao.getAllRecipes()
    suspend fun insertRecipes(recipes: List<Recipe>) = recipeDao.insertRecipes(recipes.toRecipeDBEntityList())
    ...
    suspend fun searchRecipesByTitle(query: String) = recipeDao.searchRecipesByTitle(query)
    suspend fun getRandomRecipe() = recipeDao.getRandomRecipe()
    suspend fun searchRecipesByFirstLetter(query: String) = recipeDao.searchRecipesByFirstLetter(query)
    suspend fun getAllCategories() = categoryDao.getAllCategories()
    suspend fun insertCategories(categories: List<Category>) = categoryDao.insertCategories(categories.toCategoryDBEntityList())
    ...
    suspend fun searchCategoriesByFirstLetter(query: String) = categoryDao.searchCategoriesByFirstLetter(query)
}